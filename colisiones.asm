...
reset_ball:
        ...
        ld (CoordFrac+1), a
        ld hl, -256
        ld (Vector), hl         ; one cell per frame upward
        ld hl, 256
        ld (Vector+2), hl       ; and one to the right
        ret
...
reset_round:
        call reset_ball
        ld a, (POSICION)
        ld (POSICION+1), a      ; make the paddle's CURRENT column the "previous" one so
                                ; the next dibujarpala erases it before redrawing at the
                                ; centre. Leaving a stale value here erases a column the
                                ; paddle no longer occupies.
        ld a, PADDLE_START
        ld (POSICION), a
        ret
...
; ----------------------------------------------------------------------------------------
                ;   row velocity     column velocity
rebound_table:
        DW -128  : DW -256          ; 0  far left    shallow, hard left
        DW -160  : DW -192          ; 1
        DW -224  : DW -128          ; 2
        DW -256  : DW   64          ; 3  centre      steep; sign follows the incoming ball
        DW -224  : DW  128          ; 4
        DW -160  : DW  192          ; 5
        DW -128  : DW  256          ; 6  far right   shallow, hard right
